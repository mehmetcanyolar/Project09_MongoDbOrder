...
namespace Project09_MongoDbOrder.Services
{
    public class OrderOperation
    {
        ...
        {
            var connection = new MongoDbConnection(); //yeni bir connection nesnesi olusturdum
            var orderCollection = connection.GetOrdersCollection(); // bir tane order collection(table) object olusturdum

            var document = new BsonDocument
            {
                //{"OrderId",order.OrderId },          order class ın da attribute olarak BSonId & Bson Represatation ı Id olaraktanımladım ki otomatik artsın diye
